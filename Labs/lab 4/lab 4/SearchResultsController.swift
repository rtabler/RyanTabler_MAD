...
class SearchResultsController: UITableViewController, UISearchResultsUpdating {
   ...
   func updateSearchResults(for searchController: UISearchController) {
      ...
      if searchString?.isEmpty == false {
         ...
         let searchfilter: (String) -> Bool = { name in // name is the word in question
            //look for the search string as a substring of the word
            let range = name.range(of: searchString!, options: .caseInsensitive)
            return range != nil //returns true if the value matches the search string
            // it would presumably return the index if it finds searchString in name
         } //end closure
